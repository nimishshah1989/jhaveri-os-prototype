...
export const GLOSSARY: Record<string, Term> = {
  ...
  client_value: {
    ...
    means: "The market value of everything this client holds with you, against what they put in.",
    read: 'The gap between the two is their gain — but read it next to the return, because a large gain on money invested long ago may still be behind the market.',
    tag: 'computed',
    sources: ['fifo_summary_holding.sh_current_value', '.sh_invested_value'],
    pages: ['Client 360'],
  },
  journey: {
    label: 'Investment journey',
    means: 'What this client actually put in over time, against what it is worth now and what the benchmark would have done with the same money on the same dates.',
    read: 'The benchmark line uses their own contribution dates, not a lump sum on day one — so it is a fair comparison rather than a flattering one.',
    act: 'If the portfolio line has been under the benchmark line for more than a year, that is a conversation, not a wait.',
    tag: 'computed',
    sources: ['transaction_master.tr_date', '.tr_amount', 'benchmark_price_history.price'],
    pages: ['Client 360'],
  },
  tax_position: {
    label: 'Tax position',
    means: 'Gains already realised this financial year, and gains still unrealised, each split into long and short term.',
    read: 'Short-term gains are taxed harder. Unrealised long-term gains below the annual exemption are the ones worth harvesting before March.',
    act: 'Check the exemption headroom before advising any redemption — selling in the wrong order costs the client real money.',
    formula: 'FIFO lot matching, honouring the 31-Jan-2018 grandfathered price and the exit-load window.',
    tag: 'computed',
    sources: ['fifo_summary_holding.sh_realized_ltcg', '.sh_realized_stcg', '.sh_unrealized_ltcg', '.sh_unrealized_stcg'],
    pages: ['Client 360'],
    caveat: 'It shows the position, not tax advice. The client\'s other income decides the actual rate.',
  },
  fund_verdict: {
    label: 'Fund verdict',
    means: 'Whether each fund is keeping up with the benchmark it is measured against.',
    read: '"Lagging" means it is more than 10 percentage points behind over the holding period — not that it had a bad month.',
    act: 'Do not switch on one verdict alone. Check how long they have held it and what exiting would cost in tax first.',
    tag: 'rule',
    sources: ['fifo_summary_holding.sh_xirr', '.sh_bmxirr'],
    pages: ['Client 360'],
  },
  lookthrough: {
    label: 'What they actually own',
    means: 'The underlying shares inside their funds, added up across every fund they hold.',
    read: 'Two funds with different names often hold the same companies. This shows how much of their money is really riding on a handful of stocks.',
    act: 'If one sector or a small set of stocks carries most of the money, that is a diversification conversation even though they hold many funds.',
    tag: 'computed',
    sources: ['mf_scheme_holdings.weight_pct', 'stock_master.sector'],
    pages: ['Client 360'],
    caveat: 'Equity funds only. Debt, liquid, hybrid and gold have no stock look-through, and the page says so rather than guessing.',
  },
  risk_fit: {
    label: 'Risk fit',
    means: 'Where the portfolio actually sits against the risk profile the client agreed to.',
    read: 'A portfolio to the right of the client marker is carrying more risk than they signed up for.',
    act: 'A mismatch is a suitability issue. Either the profile is out of date or the portfolio has drifted — find out which before it becomes a complaint.',
    tag: 'rule',
    sources: ['client_master_mf_related.risk_profile', 'scheme_master.risk_level'],
    pages: ['Client 360'],
    caveat: 'The allocation bands behind this are pending a compliance decision, so the scale shows position, not a pass or fail.',
  },
  holdings_lots: {
    label: 'Holdings and lots',
    means: 'Every fund they hold, and underneath it every individual purchase still open.',
    read: 'The lot level is what tax is actually calculated on. A fund can be in profit overall while individual lots are in loss.',
    act: 'When planning a redemption, look at the lots, not the fund total — which lots go first decides the tax.',
    tag: 'computed',
    sources: ['fifo_summary_holding_active.balance_units', 'fifo_detail_holding_latest.dhl_purchase_date'],
    pages: ['Client 360'],
  },
  client_consents: {
    label: 'Consents',
    means: 'What this client has agreed you may contact them about, on which channel.',
    read: 'Consent is per channel and per purpose. Agreeing to statements on WhatsApp is not agreeing to marketing on it.',
    act: 'If a channel says withdrawn, do not re-ask. Ask about a different channel at the next review instead.',
    tag: 'rule',
    sources: ['consents.channel', '.purpose', '.state', '.captured_via'],
    pages: ['Client 360', 'Campaigns'],
  },
  household: {
    label: 'Household',
    means: 'Everyone in this client\'s family group, and what the group holds together.',
    read: 'Families make money decisions together. The household total is often the number worth talking about, not the individual one.',
    tag: 'computed',
    sources: ['family_master', 'client_master.fk_family_id', '.is_family_head'],
    pages: ['Client 360'],
  },
  decision_trail: {
    label: 'Decision trail',
    means: 'What was suggested for this client, what you chose to do, and how it ended.',
    read: 'This is the record that shows advice was given and acted on — or declined, with the reason.',
    act: 'Record the outcome even when nothing happened. A declined suggestion with a reason is worth more later than a silent one.',
    tag: 'computed',
    sources: ['events.event_type', 'actions.outcome_type', '.created_from'],
    pages: ['Client 360'],
  },
