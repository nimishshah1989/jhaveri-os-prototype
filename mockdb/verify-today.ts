...
check('Churn card count == churn click-through rows', churnRisk().value.n === churnList().rows.length);
check('Idle card count == idle list total', idleNoSip().value.n === idleList().total);
check('SIP-risk card count == its list rows', sipsAtRisk().value.n === sipRiskList().rows.length);
